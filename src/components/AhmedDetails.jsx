...
const AhmedDetails = ({ route, navigation }) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`https://cdn.jsdelivr.net/gh/aashif000/DB-for-Q-H@main/ahmed/${chapterId}.json`);
  ...
  const filteredHadiths = data?.hadiths?.filter(hadith => {
    const englishText = hadith.english.text.toLowerCase();
    const narrator = hadith.english.narrator.toLowerCase();
    const query = searchQuery.toLowerCase();
    return englishText.includes(query) || narrator.includes(query);
  });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <TextInput
          ...
          placeholder="Search this Hadith"
          value={searchQuery}
