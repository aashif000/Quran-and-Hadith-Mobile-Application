...
const Nawawi = ({ navigation }) => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['nawawiAll'],
    queryFn: async () => {
      const response = await fetch('https://cdn.jsdelivr.net/gh/aashif000/DB-for-Q-H@main/nawawi40/all.json');
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <View style={styles.heroSection}>
          ...
          <Text style={styles.author}>{data?.metadata?.english?.author}</Text>
          <Text style={styles.description}>
            An-Nawawi's Forty Hadith is a compilation of forty fundamental hadiths by Imam An-Nawawi, covering the core aspects of Islamic faith, practice, and ethics.
          </Text>
        </View>
  ...
  {data?.hadiths?.map((hadith, index) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('NawawiDetails', { hadithId: hadith.id })}
    >
